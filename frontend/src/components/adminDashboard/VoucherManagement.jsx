...
const FIXED_MIN = 1_000;
const FIXED_MAX = 1_000_000;

const normalizeDiscountType = (raw) => {
  if (!raw) return 'Fixed';
...
const isPercentType = (raw) => normalizeDiscountType(raw) === 'Percent';

const discountTypeLabel = (raw) => (
  isPercentType(raw) ? 'Giảm theo phần trăm' : 'Giảm theo số tiền cố định'
);
...
const VoucherManagement = () => {
  ...
  const openEdit = (voucher) => {
    ...
    const typeNormalized = normalizeDiscountType(voucher.discountType || 'Percent');
    setFormData({
      code: voucher.code || '',
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    const typeNormalized = normalizeDiscountType(formData.discountType);
    ...
    if (typeNormalized !== 'Percent' && typeNormalized !== 'Fixed') {
    ...
    if (typeNormalized === 'Percent') {
      if (discountValue < 1 || discountValue > 100) {
        toast.error('Giá trị giảm theo % phải từ 1–100');
        return;
      }
    } else if (discountValue < FIXED_MIN || discountValue > FIXED_MAX) {
      toast.error('Giá trị giảm theo số tiền phải từ 1.000–1.000.000 (VND)');
      return;
    }
    ...
    if (minOrderAmount !== null && minOrderAmount > MAX_AMOUNT) {
      toast.error('Giá trị đơn hàng tối thiểu vượt quá giới hạn');
      return;
    }
    ...
    if (typeNormalized === 'Percent' && maxDiscountAmount !== null && maxDiscountAmount > MAX_AMOUNT) {
      toast.error('Giá trị giảm tối đa vượt quá giới hạn');
      return;
    }
    ...
    if (formData.ownerUserId && String(formData.ownerUserId).length > 450) {
      toast.error('OwnerUserId không được vượt quá 450 ký tự');
      return;
    }
    if (formData.source && String(formData.source).length > 100) {
      toast.error('Source không được vượt quá 100 ký tự');
      return;
    }
                ...
                return (
                  <tr key={id || voucher.code} className="border-b hover:bg-gray-50 text-sm">
                    ...
                    <td className="py-3 px-4 text-gray-700 capitalize">
                      <div>{voucher.discountType ? discountTypeLabel(voucher.discountType) : '--'}</div>
            ...
            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, discountType: normalizeDiscountType(e.target.value) })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
                >
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={isPercentType(formData.discountType) ? 1 : FIXED_MIN}
                    max={isPercentType(formData.discountType) ? 100 : FIXED_MAX}
                    step={1}
                  ...
                  <span className="text-sm text-gray-600 whitespace-nowrap">
                    {isPercentType(formData.discountType) ? '%' : 'VND'}
                  </span>
                </div>
              </div>
              <div>
              ...
              {isPercentType(formData?.discountType) && (
                <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold">{discountTypeLabel(detailVoucher.discountType)}</p>
                    </div>
                    <div>
