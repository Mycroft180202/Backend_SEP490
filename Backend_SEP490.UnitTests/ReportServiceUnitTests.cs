...
namespace Backend_SEP490.UnitTests
{
    public class ReportServiceUnitTests
    {
        ...
        // ==================================================================
        // DATA ANNOTATION VALIDATION TESTS
        // ==================================================================

        [Theory(DisplayName = "ReportProductRequest - TargetProductId is required")]
        [InlineData(null, false)]
        [InlineData("", false)]
        [InlineData("   ", false)]
        [InlineData("PROD-001", true)]
        public void ReportProductRequest_TargetProductId_Validation(string? id, bool expected)
        {
            var request = new ReportProductRequest
            {
                TargetProductId = id ?? string.Empty,
                Reason = "Valid reason"
            };
        ...
        [Theory(DisplayName = "ReportProductRequest - Reason required and max 500 chars")]
        [InlineData(null, false)]
        [InlineData("", false)]
        [InlineData("Valid", true)]
        [MemberData(nameof(ReportReasonTestData))]
        public void ReportProductRequest_Reason_Validation(string? reason, bool expected)
        {
            var request = new ReportProductRequest
            {
                TargetProductId = "P1",
        ...
        [Theory(DisplayName = "UpdateReportStatusRequest - NewStatus required and max 50 chars")]
        [InlineData(null, false)]
        [InlineData("", false)]
        [InlineData("Resolved", true)]
        [MemberData(nameof(NewStatusTestData))]
        public void UpdateReportStatusRequest_NewStatus_Validation(string? status, bool expected)
        {
            var request = new UpdateReportStatusRequest { NewStatus = status };
            var isValid = Validator.TryValidateObject(request, new ValidationContext(request), null, true);
            Assert.Equal(expected, isValid);
        }

        [Fact(DisplayName = "ReportProductRequest - Full valid model passes")]
        public void ReportProductRequest_FullValid_Passes()
        {
            var request = new ReportProductRequest
            {
                TargetProductId = "PROD-123",
                Reason = "Sản phẩm vi phạm bản quyền"
            };
        ...
        public static IEnumerable<object[]> NewStatusTestData => new List<object[]>
        {
            new object[] { null, false },
            new object[] { "", false },
            new object[] { "Resolved", true },
            new object[] { new string('X', 50), true },
            new object[] { new string('X', 51), false },
        };
        public static IEnumerable<object[]> ReportReasonTestData => new List<object[]>
        {
            new object[] { null, false },
            new object[] { "", false },
            new object[] { "   ", false },
            new object[] { "Sản phẩm giả mạo", true },
